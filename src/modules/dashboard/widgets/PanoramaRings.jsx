...
function Ring({ pct, color, size = 44 }) {
  ...
  return (
    <svg width={size} height={size} className="flex-shrink-0 -rotate-90">
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--bg-surface-2)" strokeWidth="4" />
      <circle
...
export function PanoramaRings() {
  ...
  const waterGoal = useHealthStore(s => s.plans.water?.dailyGoalMl || s.plans.goals?.waterDailyMl || 2500);
  const workoutLogToday = useHealthStore(s => s.workoutLog[t] || {});

  const xpLogs = useGameStore(s => s.xpLogs);
  ...
  const stats = useMemo(() => {
    ...
    return { habitsPct, habitsDone, habitsTotal, todayXP, healthPct, todayExpense, focusPct, todayMinutes };
  }, [habitsPlan, habitLogToday, waterLog, waterGoal, workoutLogToday, xpLogs, transactions, todayMinutes, t]);
        ...
        >
          <div className="relative flex items-center justify-center w-11 h-11">
            {c.pct !== null ? (
              ...
              <div className="w-11 h-11 rounded-full flex items-center justify-center text-lg" style={{ background: `${c.color}18` }}>
                {c.icon}
              </div>
            )}
          </div>
