// SessionQuickModal — Modal global de sessão de estudo (desacoplado do módulo study)
// Caminho original: modules/study/components/SessionQuickModal.jsx
// Motivo da mudança: permitir React.lazy no módulo study sem quebrar o modal global
        ...
        >
          ...
          {phase === 'finish' && (
            <div className="p-5 space-y-5 max-h-[84vh] overflow-y-auto">
              ...
              {revisionId && (
                <div>
                  <Label>Score da Revisão</Label>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { value: 1, label: 'Difícil', emoji: '😫', color: '#EF4444' },
                      { value: 3, label: 'Médio', emoji: '🤔', color: '#F59E0B' },
                      { value: 5, label: 'Fácil', emoji: '😊', color: '#10B981' },
                    ].map(opt => (
                      <button
                        key={opt.value}
                        onClick={() => setRevisionScore(opt.value)}
                        className="py-2.5 rounded-xl border text-xs font-bold transition-all text-center"
                        style={{
                          borderColor: revisionScore === opt.value ? opt.color : 'var(--border)',
                          background: revisionScore === opt.value ? `${opt.color}18` : 'var(--bg-surface-2)',
                          color: revisionScore === opt.value ? opt.color : 'var(--text-muted)',
                        }}
                      >
                ...
                <div className="flex flex-wrap gap-2">
                  {MODES.map(m => (
                    <button
                      ...
                      className="px-3 py-2 rounded-xl border text-xs font-bold transition-all"
                      style={{
                        borderColor: has(m.id) ? m.color : 'var(--border)',
                        background: has(m.id) ? `${m.color}18` : 'var(--bg-surface-2)',
                        color: has(m.id) ? m.color : 'var(--text-muted)',
                      }}
                    >
                      {m.icon} {m.label}
                    </button>
              ...
              {has('questoes') && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <NI placeholder="0" value={qAnswered} onChange={setQAnswered} />
                  </div>
                  <div>
                    <Label>Questões corretas</Label>
                    <NI placeholder="0" value={qCorrect} onChange={setQCorrect} />
                  </div>
                </div>
              )}
              ...
              {/* conclusão */}
              <div>
                <Label>Você sente que dominou?</Label>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => setConcluded(true)}
                    className="py-2.5 rounded-xl border text-xs font-bold transition-all"
                    style={{
                      borderColor: concluded === true ? '#10B981' : 'var(--border)',
                      background: concluded === true ? '#10B98118' : 'var(--bg-surface-2)',
                      color: concluded === true ? '#10B981' : 'var(--text-muted)',
                    }}
                  >
                    ✅ Dominei
                  </button>
                  <button
                    ...
                    className="py-2.5 rounded-xl border text-xs font-bold transition-all"
                    style={{
                      borderColor: concluded === false ? '#F59E0B' : 'var(--border)',
                      background: concluded === false ? '#F59E0B18' : 'var(--bg-surface-2)',
                      color: concluded === false ? '#F59E0B' : 'var(--text-muted)',
                    }}
                  >
                    ⚠️ Ainda estudando
                  </button>
              ...
              {concluded !== false && (
                <div>
                  <Toggle
                    ...
                    accent="#06B6D4"
                    label="Gerar revisões espacadas"
                    sub="Agenda revisões nos dias 1, 3, 7 e 21"
                  />
                </div>
              )}
              ...
              <button
                onClick={handleSave}
                className="w-full py-3.5 rounded-xl font-bold text-sm text-white uppercase tracking-wider transition-all hover:opacity-90"
                style={{ background: accentSafe }}
              >
                💾 Salvar sessão
              </button>
          ...
          {phase === 'result' && result && (
            <div className="p-6 flex flex-col items-center gap-4 text-center">
              <div
                ...
                style={{ color: result.subjectColor }}
              >
                +{result.xpEarned} XP
              </div>
